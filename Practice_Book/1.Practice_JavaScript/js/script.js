...
// * Технически HTML код аккордеона удобнее, чем код вкладок:
// * в нем ссылки расположены непосредственно над спойлером, который
// * они открывают и закрывают. Кроме того, класс active в нашей верстки дается
// * общему родителю ссылки и спойлера.
// * Это значит, что при нажатии на ссылку нужно просто деактивировать
// * старую вкладку и активировать новую. Чтобы деактивировать старую вкладку нужно
// * найти вкладку с классом active и удалить из нее этот класс.
// * Чтобы активировать новую вкладку нужно по клику на ссылку найти вкладку-родителя
// * этой ссылки и дать ей класс active. Технически такой поиск можно сделать так: просто
// * найти ближайшего родителя ссылки, имеющего класс tab.


let tabLink = document.querySelectorAll('.tabAk .link a');

for (const link of tabLink) {
    link.addEventListener('click', (ev) => {
        ev.preventDefault();
        let activeTab = document.querySelector('.tabAk.active');
        if (activeTab) {
            activeTab.classList.remove('active');
        }
        let akTab = link.closest('.tabAk');

        if (akTab !== activeTab) {
            akTab.classList.add('active');
        }
    })
}

// почти сделал сам но забыл про метод closets и пришлось подсмотреть
